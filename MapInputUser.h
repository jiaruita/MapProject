...
BEGIN: printf("Enter start (return to quit): "); fgets(s,MaxString,stdin);
if (s[0]=='\n') return(0);
Begin = INDEX(s);
...
/*Get finish location.*/
FINISH: printf("Enter finish (return to quit): "); fgets(s,MaxString,stdin);
if (s[0]=='\n') return(0);
Finish = INDEX(s);
...
/*Get BoardFlag.*/
printf("Have a skateboard (y/n - default=n)? "); fgets(s,MaxString,stdin);
BoardFlag = ( (s[0]=='y') || (s[0]=='Y') || (strncmp(s,"yes",3)==0) || (strncmp(s,"Yes",3)==0) ) ? 1 : 0;

/*Get TimeFlag.*/
printf("Minimize time (y/n - default=n)? "); fgets(s,MaxString,stdin);
